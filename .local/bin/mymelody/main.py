...
def _download_url(resp):
    ...
    title = (resp["name"],)
    try:
        yid = ytm.search(f"{artist} {album} {title}", filter="songs")[0][
            "videoId"
        ]
...
@click.pass_context
def main(ctx):
    """Download songs from Spotify via Youtube"""
    ctx.obj = {}
    ctx.obj["debug"] = False
    ctx.obj["config_path"] = (
        os.getenv("XDG_CONFIG_HOME", "HOME") + "/mymelody/config.json"
    )
    ...
    if not all(
        k in ctx.obj for k in ("client_id", "client_secret", "output_dir")
    ):
    ...
    if "data_path" not in ctx.obj:
        ctx.obj["data_path"] = (
            os.getenv("XDG_DATA_HOME", "HOME") + "/mymelody/data.json"
        )
    if "cache_path" not in ctx.obj:
        ctx.obj["cache_path"] = (
            os.getenv("XDG_CACHE_HOME", "HOME") + "/mymelody/cache.json"
        )
...
@click.pass_obj
def playlist(ctx, id):
    """Collects all songs from a playlist, creating a m3u file"""
    ...
    collected_tracks = [
        track_file("..", ctx["tracks"][track_id])
        for track_id in playlist_track_ids
    ]
...
@click.pass_obj
def download(ctx, id):
    """Downloads the songs for which the metadata is already present"""
    if id:
